...
app.MapPost("/entitlements/check", async (EntitlementCheckRequest request, IEntitlementCheckService service, CancellationToken cancellationToken) =>
    {
        if (string.IsNullOrWhiteSpace(request.SubjectId)) return Results.BadRequest("subjectId is required.");
        if (string.IsNullOrWhiteSpace(request.PermissionName)) return Results.BadRequest("permissionName is required.");
        if (string.IsNullOrWhiteSpace(request.ResourceId)) return Results.BadRequest("resourceId is required.");
